...
var utilities = {
	...
	build: function (options, info, callback) {
		...
		var ids = utilities.getIds(options.ids, info.modules);
		...
		var buildSteals = utilities.getSteals(configuration.steals, ids);
		...
		pluginify.pluginifySteals(buildSteals, pluginifyOptions, function (error, str) {
			...
			var banner = info.banner ? utilities.banner(info.banner, {
				url: options.url || '',
				//added for consistency to reference pkg file via gruntfile pkg property
				pkg: info,
				info: info, //leaving this here for compatibility
				ids: ids
			}) : '';
