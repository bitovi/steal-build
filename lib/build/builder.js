 ...
 */
function loadSteals(info, config, callback) {
	...
	var stealers = _.map(configurations, function(configuration, configName) {
		return function(callback) {
			var stealConfig = utilities.deepExtend(config, configuration.steal);
...
function builder(options, info, callback) {
	...
	var pluginifyOptions = _.extend({
		opener: configuration.steals.opener
	}, info.pluginify, configuration.pluginify);
 ...
 * @returns {*}
 */
builder.loadInfo = function (options, callback) {
	...
	loadSteals(info, stealConfig, function(error, steals) {
		if(error) {
			return callback(error);
		}
