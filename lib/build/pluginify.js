...
function pluginify(ids, options, callback) {
	opener(ids, options.root, function (rootSteals) {
	...
	opener.visit(steals, function (stl, id, visited) {
		...
		if (stl.options.text) {
			...
			var part = _.template(options.moduleTemplate, {
				...
				pluginified: stl.options.parsed,
	...
	if (options.exports) {
		...
		exportModules = _.map(options.exports, function (name, moduleId) {
			return _.template(options.exportsTemplate, {
				name: name,
				module: nameMap[moduleId]
			});
		});
	}
	var pluginified = _.template(options.wrapper, {
		content: contents.join('\n'),
		exports: exportModules
	});
	callback(null, pluginified);
}
