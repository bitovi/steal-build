...
function configureSteal(options) {
	// TODO this doesn't work. Use steal.clone()
	stealBackup.config = steal.defaultConfig;
	stealBackup.root = steal.root;
	// Overwrite the steal types
	steal.config(_.extend({}, options, {
		types: {
	...
	steal.root = options.root || process.cwd();
}
